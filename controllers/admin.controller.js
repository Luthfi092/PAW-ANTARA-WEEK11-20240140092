...
async function login(req, res) {
  try {
    ...
    if (!username || !password) {
      return sendResponse(res, { code: 400, success: false, message: 'username dan password wajib diisi' });
    }

    const admin = await Admin.findOne({ where: { username } });
    if (!admin) {
      return sendResponse(res, { code: 401, success: false, message: 'username atau password salah' });
    }
    ...
    return sendResponse(res, { message: 'Login berhasil', data: { id: admin.id, username: admin.username } });
  } catch (err) {
